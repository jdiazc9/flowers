...
% single plates
plate_singles_1 = zeros(sqrt(Nc),sqrt(Nc),Ns);
plate_singles_2 = zeros(sqrt(Nc),sqrt(Nc),Ns);

for i=1:Ns/2
    row = ceil(i/sqrt(Nc));
    col = i - sqrt(Nc)*(row-1);
    plate_singles_1(row,col,i) = 1;
    plate_singles_2(row,col,Ns/2+i) = 1;
end

% use protocols to generate communities
[C,id] = protocol(plate_singles_1,plate_singles_2);
...
function [C,id] = protocol_1(plate_singles_1,plate_singles_2)

% pairs plates
plate_pairs_1 = plate_singles_1 + plate_singles_2;

% quadruplets
plate_quad_1 = plate_pairs_1 + rotate_left(plate_pairs_1);

% octuplets
plate_oct_1 = plate_quad_1 + rotate_left(rotate_left(plate_quad_1));

% fix structure
Ns = size(plate_singles_1,3);
Nc = size(plate_singles_1,1)*size(plate_singles_1,2);
C = zeros(Ns,Nc);
for i=1:Nc
    row = ceil(i/sqrt(Nc));
    col = i - sqrt(Nc)*(row-1);
...
function [C,id] = protocol_2(plate_singles_1,plate_singles_2)

% transformation_table
transformation_table = [1,2,3,4,5,6,7,8;5,6,7,8,1,2,3,4]';

% isolates plates
plate_singles_3 = shuffle_rows(plate_singles_1,transformation_table);
plate_singles_4 = shuffle_rows(plate_singles_2,transformation_table);

% pairs
plate_pairs_1 = plate_singles_1 + plate_singles_3;
plate_pairs_2 = plate_singles_2 + plate_singles_4;
plate_pairs_3 = plate_singles_1 + plate_singles_2;
plate_pairs_4 = plate_singles_3 + plate_singles_4;

% quadruplets
plate_quad_1 = plate_pairs_1 + rotate_left(plate_pairs_2);
plate_quad_2 = plate_pairs_3 + rotate_left(plate_pairs_4);

% octuplets
plate_oct_1 = plate_quad_1 + rotate_left(rotate_left(plate_quad_2));
...
function [C,id] = protocol_3(plate_singles_1,plate_singles_2)
...
function [C,id] = protocol_4(plate_singles_1,plate_singles_2)
...
% pairs
plate_pairs_1 = plate_singles_1 + plate_singles_2;
plate_pairs_2 = rotate_left(plate_singles_1) + rotate_right(plate_singles_2);

% quadruplets
plate_quad_1 = plate_pairs_1 + plate_pairs_2;

% octuplets
plate_oct_1 = plate_quad_1 + shuffle_rows(plate_quad_1,transformation_table);
...
function [C,id] = protocol(plate_singles_1,plate_singles_2)
[C,id] = protocol_4(plate_singles_1,plate_singles_2);
end
